...
function renderInterview() {
  ...
  if(interviewList.length == 0){
    ...
    jobRepot.innerText = interviewList.length + " of " + allCardSection.children.length + " Jobs";
  }
  // 
  if(interviewList.length == 0){
    filteredSection.innerHTML = `
...
function renderRejected() {
  ...
  if(rejectList.length == 0){
    ...
    jobRepot.innerText = rejectList.length + " of " + allCardSection.children.length + " Jobs";
  }
  ...
  for (let rejected of rejectList) {
    ...
    div.innerHTML = `
      <div class="space-y-6">
                    ...
                    <p class="fastName text-4xl text-[#002C5C]">${rejected.fastName}</p>
                    <p class="developer">${rejected.developer}</p>
                    <p class="jobTime">${rejected.jobTime}</p>

                    <p class="status text-[#002C5C]">${rejected.status}</p>
